...
def iqr(matrix:np.ndarray) -> tuple[np.ndarray, dict]:
    """
    ...
    Returns:
        tuple: (cleaned_data_matrix, channel_modifications)
            channel_modifications maps original channel index to one of:
            - list of modified time-sample indices
            - 'removed' if the channel was deleted
            - 'unchanged' if no sample was modified
    """
    ...
    for ch in range(n_channels):
        ...
        if is_outlier_channel[ch]:
            ...
            if r >= 5:
                ...
                channel_modifications[ch] = 'removed'
                removed_channels += 1
            else:
                ...
                modified_samples = np.where(mask)[0].tolist()
                channel_modifications[ch] = modified_samples if modified_samples else 'unchanged'
    ...
    return clean_matrix, channel_modifications