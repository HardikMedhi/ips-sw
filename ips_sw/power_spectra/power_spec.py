...
def uniform_statistical_averaging(freqs: np.ndarray, psd: np.ndarray, k:float=4) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def get_ps(fb_obj: Filterbank, 
           ...
           matrix:np.ndarray=None) -> tuple[np.ndarray, np.ndarray]:
